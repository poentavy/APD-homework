...
// void* parallel_operations(void* arg) {
//     RescaleThreadData* data = (RescaleThreadData*)arg;
//     int step_x = STEP;
// 	int step_y = STEP;

//     // Cast the argument to RescaleThreadData
//     // Call all the necessary functions using the data in the argument
...
void* rescale_image_thread(void* arg) {
    uint8_t sample[3];
    ppm_image *image= ((RescaleThreadData*)arg)->image;
    RescaleThreadData *th= (RescaleThreadData*)arg;
    // we only rescale downwards
    if (th->scaled_image == NULL) {
        return;
    }

    ppm_image *new_image = th->scaled_image;
	int start = th->thread_id * (th->scaled_image->x / NUM_THREADS);
    int end;
    if (th->thread_id == NUM_THREADS - 1) {
        end = new_image->x;
    } else {
        end = (th->thread_id + 1) * (new_image->x / NUM_THREADS);
        }
...
int main(int argc, char *argv[]) {
    ...
    RescaleThreadData thread_data[NUM_THREADS];
	pthread_mutex_t mutex;
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, rescale_image_thread, &thread_data[i]);
    }
    ...
    // 1. Rescale the image
        ppm_image *scaled_img;
    if (thread_data[0].scaled_image == NULL) {
        scaled_img = thread_data[0].image;
    } else {
        scaled_img = thread_data[0].scaled_image;
    }


    // 2. Sample the grid
    unsigned char **grid = sample_grid(scaled_img, step_x, step_y, SIGMA);

    // 3. March the squares
    march(scaled_img, grid, contour_map, step_x, step_y);

    // 4. Write output
    write_ppm(scaled_img, argv[2]);
    pthread_mutex_destroy(thread_data->mutex);
    if (image->x > RESCALE_X || image->y > RESCALE_Y) {
		free(image->data);
		free(image);
	}
    free_resources(scaled_img, contour_map, grid, step_x);
